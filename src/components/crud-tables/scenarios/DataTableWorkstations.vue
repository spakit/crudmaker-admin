...
<script>
import { workstations as crudWorkstations } from '../../../data/crudSchema.json'
import workstations from '../../../data/workstations.json'

export default {
  data () {
    return {
      workstations: workstations.slice(0, 8),
    }
  },
  computed: {
    fields () {
      return crudWorkstations.tableFields
    },
  },
  methods: {
    edit (user) {
    ...
    remove (user) {
      const idx = this.workstations.findIndex(u => u.id === user.id)
